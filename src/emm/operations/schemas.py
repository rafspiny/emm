...
from src.emm.models.database_base import context_session, get_db_session
...
def initialize_schema(project_name: str):
    ...
    """
    # FIXME Instead of just executing the sql file, parse it and reate the entity
    ...
    with context_session() as session:
        ...
        if session.execute(
            ...
            raise Exception(
                ...
                "Please use a different anme or clean "
...
def delete_schema(schema: Schema):
    ...
    """
    with context_session() as session:
        session.execute(text(f"DROP SCHEMA IF EXISTS {schema.name}"))
