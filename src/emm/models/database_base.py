...
def _build_db_url(config: Config) -> str:
    return f"postgres://{config.emm_db_user}:{config.emm_db_pwd}@"
...
) -> Callable[[type[MODEL_TYPE]], type[MODEL_TYPE]]:
    """
    A decorator to easily create sqlalchemy models out of dataclasses
    """

    def wrap(cls: type[MODEL_TYPE]) -> type[MODEL_TYPE]:
        cls.__tablename__ = table_name  # type: ignore
        cls.__table_args__ = {"schema": schema}  # type: ignore
